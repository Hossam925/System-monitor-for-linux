...
long LinuxParser::ActiveJiffies(int pid) {
  string line,temp;
  long value,utime,stime,cutime,cstime,totaltime;
  ...
  if(myfile.is_open()){
    ...
    utime     = cpu_values[13];
    stime     = cpu_values[14];
    cutime    = cpu_values[15];
    cstime    = cpu_values[16];
...
long LinuxParser::UpTime(int pid) { 
  string pid_path = to_string(pid);
  string line,key,value;
  vector<string> clock_ticks;
  long time;
  std::ifstream myfile(kProcDirectory+pid_path+kStatFilename);
  if(myfile.is_open()){
    std::getline(myfile,line);
      std::istringstream lineStream(line);
      while(lineStream >> value){clock_ticks.push_back(value);}
    time = std::stol(clock_ticks[21])/sysconf(_SC_CLK_TCK);
  }
  return time; 
}